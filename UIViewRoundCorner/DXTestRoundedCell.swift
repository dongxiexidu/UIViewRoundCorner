...
class DXTestRoundedCell: UITableViewCell {
    ...
    override func awakeFromNib() {
       ...
       // layer.isOpaque = true
        ...
        for i in 0..<10 {
           ...
           // let color = i % 2 == 0 ? UIColor.white : UIColor.gray
            ...
            circle.dx_roundCorner(radius: 15/2, cornerColor: color)
      ...
      //  descLabel.layer.isOpaque = true
        descLabel.dx_roundCorner(radius: 20/2, cornerColor: UIColor.white, corners: [.topLeft,.topRight])
...
//        topImageV.image = img
    ...
    func colorSelected(selected:Bool) {
        ...
        descLabel.dx_roundCorner(radius: 20/2, cornerColor: color, corners: [.topLeft,.topRight])
        middleLabel.dx_roundCorner(radius: 20/2, cornerColor: color,corners: [.topLeft,.topRight, .bottomRight, .bottomLeft])
        headView.dx_roundCorner(radii: CGSize.init(width: 35, height: 35), cornerColor: color, corners: .allCorners, borderColor: UIColor.red, borderWidth: 2)
        
        circleView.backgroundColor = color
        for item in circleView.subviews {
            item.dx_roundCorner(radius: 15/2, cornerColor: color)
        }
    }
